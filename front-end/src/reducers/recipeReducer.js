...
export const fetchRecipedata = createAsyncThunk(
  ...
  async (_, { extraArgument }) => {
    const response = await fetch('https://recipe-web-app-seven.vercel.app/data/recipes');
...
export const fetchRecipeById = createAsyncThunk('recipe/fetchRecipeById', async (id) => {
  const response = await fetch(`https://recipe-web-app-seven.vercel.app/data/recipes/${id}`);
  const data = await response.json();
  return data;
});
